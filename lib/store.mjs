...
export function createStore(filename) {
  ...
  if (
    !db
      ...
      .all()
      .some((column) => column.name === "journal")
  )
    db.exec("ALTER TABLE days ADD COLUMN journal TEXT NOT NULL DEFAULT ''");
  ...
  return {
    ...
    getJournal(date) {
      getDay(date);
      return db.prepare("SELECT journal FROM days WHERE date=?").get(date)
        .journal;
    },
    setJournal(date, text) {
      ...
      return transaction(() => {
        getDay(date);
        db.prepare("UPDATE days SET journal=? WHERE date=?").run(text, date);
        return text;
      });
    },
    setComplete(date, id, done, optionId) {
