  ...
  return (
    ...
    options.every(
      (o) =>
        ...
        o.label.length <= 40 &&
        (o.score === undefined ||
          (Number.isInteger(o.score) && o.score >= 0 && o.score <= 1000)),
    ) &&
...
export function optionScore(task, option) {
  return option.score ?? task.score;
}
export function maximumScore(task) {
  const options = taskOptions(task);
  return options.length
    ? Math.max(...options.map((o) => optionScore(task, o)))
    : task.score;
}
export function earnedScore(task) {
  if (!task.done) return 0;
  const options = taskOptions(task);
  if (!options.length) return task.score;
  const option = options.find((o) => o.id === selectedOption(task));
  return option ? optionScore(task, option) : 0;
}
