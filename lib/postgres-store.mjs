import { defaultTasks, validDate, validTasks } from "./task-model.mjs";
...
export function createPostgresStore(sql) {
    ...
    if (!ready)
      ready = sql
        .begin(async (tx) => {
          ...
          await tx.unsafe(
            "INSERT INTO oneday.intentions (id,tasks) VALUES (1,$1::jsonb) ON CONFLICT (id) DO NOTHING",
  ...
  async function withDay(date, action) {
    ...
    return sql.begin(async (tx) => {
      ...
      await tx.unsafe("SELECT id FROM oneday.intentions WHERE id=1 FOR UPDATE");
      await tx.unsafe(
  ...
  return {
    ...
    async setComplete(date, id, done, prayerMode) {
      ...
      return withDay(date, async (tx, tasks) => {
        ...
        await tx.unsafe(
          "UPDATE oneday.days SET tasks=$1::jsonb WHERE date=$2",
    ...
    async configure(date, tasks) {
      ...
      return withDay(date, async (tx, old) => {
        ...
        await tx.unsafe(
          "UPDATE oneday.intentions SET tasks=$1::jsonb WHERE id=1",
