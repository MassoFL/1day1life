...
// No browser receives database credentials.
export function createPostgresStore(sql) {
    ...
    if (!ready)
      ready = sql
        .begin(async (tx) => {
          ...
          await tx.unsafe(
            "ALTER TABLE oneday.days ADD COLUMN IF NOT EXISTS journal TEXT NOT NULL DEFAULT ''",
          );
          await tx.unsafe(
  ...
  return {
    ...
    async getJournal(date) {
      return withDay(date, async (tx) => {
        const [row] = await tx.unsafe(
          "SELECT journal FROM oneday.days WHERE date=$1",
          [date],
        );
        return row.journal;
      });
    },
    async setJournal(date, text) {
      if (typeof text !== "string" || text.length > 20000)
        throw new Error("Invalid journal");
      return withDay(date, async (tx) => {
        await tx.unsafe("UPDATE oneday.days SET journal=$1 WHERE date=$2", [
          text,
          date,
        ]);
        return text;
    ...
    async configure(date, tasks) {
      ...
      return withDay(date, async (tx, old) => {
        const { clean, today } = configureTasks(tasks, old);
        await tx.unsafe(
          "UPDATE oneday.intentions SET tasks=$1::text::jsonb WHERE id=1",
